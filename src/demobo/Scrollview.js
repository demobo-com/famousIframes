...
define(function(require, exports, module) {
    ...
    function _bindEvents() {
        ...
        this._eventInput.on('start', function(event){
            localStorage.setItem("scrollviewStart", JSON.stringify(event));
        });
        this._eventInput.on('update', function(event){
            localStorage.setItem("scrollviewMove", JSON.stringify(event));
        });
        this._eventInput.on('end', function(event){
            localStorage.setItem("scrollviewEnd", JSON.stringify(event));
        });
        this._scroller.on('edgeHit', function(data) {
            localStorage.setItem("scrollviewEdgeHit", JSON.stringify(data));
        }.bind(this));

        var self = this;
        window.addEventListener("storage", function(evt) {
            if (evt.key=="scrollviewStart") {
                var scrollEvt = JSON.parse(evt.newValue);
                _handleStart.call(self, scrollEvt);
            } else if (evt.key=="scrollviewMove") {
                var scrollEvt = JSON.parse(evt.newValue);
                _handleMove.call(self, scrollEvt);
            } else if (evt.key=="scrollviewEnd") {
                var scrollEvt = JSON.parse(evt.newValue);
                _handleEnd.call(self, scrollEvt);
            } else if (evt.key=="scrollviewEdgeHit") {
                var data = JSON.parse(evt.newValue);
                self._edgeSpringPosition = data.position;
            }
        });
    }
